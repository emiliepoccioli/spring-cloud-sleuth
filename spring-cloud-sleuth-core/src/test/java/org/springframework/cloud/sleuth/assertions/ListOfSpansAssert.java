...
public class ListOfSpansAssert extends AbstractAssert<ListOfSpansAssert, ListOfSpans> {
	...
	public ListOfSpansAssert everyParentIdHasItsCorrespondingSpan() {
		isNotNull();
		printSpans();
		List<String> parentSpanIds = this.actual.spans.stream().flatMap(span -> span.getParents().stream())
				.distinct().map(Span::idToHex).collect(Collectors.toList());
		List<String> spanIds = this.actual.spans.stream()
				.map(span -> Span.idToHex(span.getSpanId())).distinct()
